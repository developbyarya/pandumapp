...
export default function Pilihan({
  ...
  return (
    ...
    >
      {isAksara ? (
        <div className="rounded-xl bg-blue-800 justify-center  relative w-52 h-52 m-16 flex flex-col ">
          <h6
            className={`${Font.className} text-white text-[98px] self-center`}
          >
            {pertanyaan}
          </h6>
        </div>
      ...
      <div className="w-full grid grid-cols-2 justify-between justify-items-center">
        {option.map((opsi, index) => (
          ...
          >
            <h6 className={`text-[72px] self-center ${Font.className}`}>
              {opsi}
            </h6>
          </button>
        ))}
